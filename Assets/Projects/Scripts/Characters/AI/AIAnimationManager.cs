    ...
    public class AIAnimationManager : CharacterAnimationManager
    {
        ...
        public void HandleAnimation(float maxDistance)
        {
            if(aiManager.DistanceToTarget >= maxDistance)
            {
                ...
                {
                    aiManager.aIAnimationManager.SetBlendTreeParameter(0f, 2.0f, true, Time.deltaTime);
                    return;
                }
                aiManager.aIAnimationManager.SetBlendTreeParameter(0f, 0.55f, false, Time.deltaTime);
            }
        }
    }
}
